...
class CourseAdapter(private val clickListener: (Course) -> Unit) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CourseViewHolder {
        val binding = LayoutInflater.from(parent.context).inflate(R.layout.item_course, parent, false)
        return CourseViewHolder(binding)
    }

    override fun onBindViewHolder(holder: CourseViewHolder, position: Int) {
        val itemCourse = getItem(position) as Course
        holder.bind(itemCourse, clickListener)
    }
}